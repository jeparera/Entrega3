...
class Game():
    def __init__(self,manager,baraja):
        ...
        self.jugadores=manager.list([Jugador(UP_PLAYER), Jugador(DOWN_PLAYER)])
        self.baraja = baraja
    ...
    def jugar_recoger(self,cartasmesa,cartamano,player):
        self.mutex.acquire()
        p = self.jugadores[player]
        p.jugar_recoger(cartasmesa,cartamano,self.mesa)
        self.jugadores[player] = p
        self.mutex.release()
...
def main(ip_address,port):
    ...
    try:
        with Listener((ip_address, port),
            ...
            game.reponer_mesa()
            game.robar(3,UP_PLAYER)
            game.robar(3,DOWN_PLAYER)
            while True:
